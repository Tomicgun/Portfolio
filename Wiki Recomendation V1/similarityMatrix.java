...
public class similarityMatrix {
    ...
    public Double tf(HashMapP freqTable, ArrayList<String> webText, String word, int documentIndex) {
        int num = 0;
        outer:
        for (int i = 0; i < freqTable.table.length; ++i) {
            for (HashMapP.Node e = freqTable.table[i]; e != null; e = e.next) {
                if(word.equals(e.key)){
                    num = e.document[documentIndex]; //Set num to the number of occurences of the word in the given document
                    break outer; //CHANGE HERE
                }
            }
        }
        return (double) num / webText.size();
    }

    //Calculates the idf value of a term within the corpus
    public Double idf(HashMapP freqTable, String word) {
        int numOfDocsWithWord = 0;
        for (int i = 0; i < freqTable.table.length; ++i){
            for (HashMapP.Node e = freqTable.table[i]; e != null; e = e.next){
                if(word.equals(e.key)){ //CHANGE HERE
                    for(int docCount : e.document){
                        if(docCount != 0){
                            numOfDocsWithWord++;
                        }
                    }
        ...
        return Math.log(10.0 / numOfDocsWithWord);
    }

    //Calculates the tf-idf value of the web page's text amongst the list of strings from all the web pages
    public ArrayList<Double> tfIdf(HashMapP freqTable, ArrayList<String> webText, ArrayList<ArrayList<String>> allText) {
    ...
    public ArrayList<String> similarLinks(String link1, ArrayList<String> allLinks, HashMapP freqTable) throws IOException{
        ...
        for(int i = 0; i < allLinks.size(); i++){
            if (allLinks.get(i).equals(link1)){
                tfIdf1 = tfIdf(freqTable, parsedLinks.get(i), parsedLinks);
                break;
            }
        }
